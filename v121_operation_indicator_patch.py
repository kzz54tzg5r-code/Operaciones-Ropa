...
(function(){
 ...
 roleLabel.colaborador='Colaborador operativo';roleLabel.colaborador_operativo='Colaborador operativo';if(typeof OPER_TITLES==='object')OPER_TITLES[OP]='Control diario, productividad por persona y eficiencia de mercancía';
 ...
 function addCollaboratorRoles(){const nr=$('#newRole');if(nr){Array.from(nr.options).filter(o=>o.value==='colaborador').forEach(o=>o.remove());if(!Array.from(nr.options).some(o=>o.value==='colaborador_operativo'))nr.insertBefore(new Option('Colaborador operativo','colaborador_operativo'),nr.firstChild)}}addCollaboratorRoles();const oldLoadUsers=window.loadUsers;if(typeof oldLoadUsers==='function')window.loadUsers=async function(){await oldLoadUsers();addCollaboratorRoles();try{const users=await api('/api/users'),rows=[...document.querySelectorAll('#userTable tr')];rows.forEach((tr,i)=>{const sel=tr.querySelector('.userEditRole');if(!sel)return;Array.from(sel.options).filter(o=>o.value==='colaborador').forEach(o=>o.remove());if(!Array.from(sel.options).some(o=>o.value==='colaborador_operativo'))sel.add(new Option('Colaborador operativo','colaborador_operativo'));if(users[i])sel.value=users[i].role})}catch(e){console.warn('[V121] roles usuarios',e)}};const createBtn=$('#createUser');if(createBtn&&createBtn.onclick){const old=createBtn.onclick;createBtn.onclick=async function(ev){if($('#newRole')?.value==='colaborador_operativo'&&!$('#newStore')?.value){$('#userMsg').textContent='Selecciona la tienda del colaborador operativo';return}return old.call(this,ev)}}
