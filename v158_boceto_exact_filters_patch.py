"""V158 · Barra de filtros visible y layout alineado a boceto.

Objetivo: que los módulos se vean como los bocetos aprobados sin tocar tablas,
gráficas, cálculos ni exportaciones. V158 crea una barra visual compacta usando
los selectores reales ya existentes como fuente de verdad. No usa observers ni
intervalos; sólo reacciona a navegación/clics del usuario.
"""
...
/* Filtro visual único estilo boceto */
...
.v158-grid{display:grid;grid-template-columns:1.25fr 1fr 1fr 1fr auto;gap:7px;align-items:end}
...
.v158-quick{display:flex;gap:4px;justify-content:flex-end;margin-top:6px}.v158-chip{border:1px solid #d4e2f1;background:#f7faff;color:#496b8f;border-radius:7px;padding:4px 7px;font-size:7px;font-weight:900}.v158-chip.active{background:#e9f2ff;color:#1769e8;border-color:#a9c8ee}
...
/* Oculta todos los sistemas previos de filtros para que exista sólo V158. */
...
/* Pestañas visualmente iguales al boceto; no cambia eventos ni orden. */
...
/* Tarjetas más compactas, como boceto. */
...
/* No tocar tablas, gráficas, canvas, svg ni botones de descarga. */
...
(function(){
  ...
  const defs={
    cambios:[['Vista','operPeriodSelect'],['Tienda','operStoreSelect'],['Área','operAreaSelect'],['Actividad','operActivitySelect']],
    operation:[['Periodo','opPeriod'],['Tienda','opStore'],['Área','opArea'],['Actividad','opActivity']],
  ...
  function ensureHost(){
    ...
    h=document.createElement('div');h.id='v158FilterBar';h.innerHTML='<div class="v158-grid" id="v158Grid"></div><div class="v158-quick" id="v158Quick"><span class="v158-chip">Hoy</span><span class="v158-chip">Semana</span><span class="v158-chip">Mes</span><span class="v158-chip">Año</span></div><div id="v158MorePanel"></div>';
  ...
  function cloneSelect(label,id){
    const s=$(id);if(!s)return null;const w=document.createElement('div');w.className='v158-field';const l=document.createElement('label');l.textContent=label;const c=s.cloneNode(true);c.removeAttribute('id');c.dataset.source=id;c.value=s.value;c.addEventListener('change',()=>{const src=$(id);if(src){src.value=c.value;src.dispatchEvent(new Event('change',{bubbles:true}))}});w.append(l,c);return w;
  }
  function apply(){
    const mod=module();document.body.dataset.v158Module=mod;document.body.classList.add('v158-ready');const h=ensureHost();const grid=$('v158Grid'),more=$('v158MorePanel');grid.innerHTML='';more.innerHTML='';
    if(!defs[mod]){h.classList.remove('on');return}
    h.classList.add('on');let count=0;
    defs[mod].forEach((d,i)=>{const el=cloneSelect(d[0],d[1]);if(!el)return;count++;if(i<4)grid.appendChild(el);else more.appendChild(el)});
    const acts=document.createElement('div');acts.className='v158-actions';
    const go=document.createElement('button');go.type='button';go.className='v158-apply';go.textContent='⌕  Consultar';go.onclick=()=>{const id=mod==='analysis'?'refresh':mod==='cambios'?'operPeriodApply':'opApply';const b=$(id);if(b)b.click();else defs[mod].forEach(d=>$(d[1])?.dispatchEvent(new Event('change',{bubbles:true})));setTimeout(apply,150)};
    const mb=document.createElement('button');mb.type='button';mb.className='v158-more';mb.textContent='☷  Más filtros';mb.onclick=()=>h.classList.toggle('v158-more-open');acts.append(go,mb);grid.appendChild(acts);
    if(!count)h.classList.remove('on');
  }
  ...
  console.info('[V158] Boceto visual + filtro único visible activo.');
})();
</script>'''
    ...
    async def _v158_html(request, call_next):
        ...
        try:
            ...
            headers.update({'Cache-Control':'no-store, no-cache, must-revalidate, max-age=0','Pragma':'no-cache','Expires':'0','X-Operations-UI-Version':'V158-BOCETO-FILTERS'})
            return HTMLResponse(html,status_code=response.status_code,headers=headers)
        except Exception as exc:
            print(f'[V158] HTML warning: {type(exc).__name__}: {exc}',flush=True);return response
    m._V158_BOCETO_EXACT_FILTERS=True
    print('[V158] Boceto visual y filtro único visible instalados; reportes y exportaciones intactos.',flush=True)
