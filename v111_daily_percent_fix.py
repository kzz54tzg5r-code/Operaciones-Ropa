"""V111 final: porcentajes en Operación Diaria (web + PDF).

- Tarjeta Acondicionado: muestra % Acondicionado como valor principal y piezas como detalle.
- Tarjeta Ubicado: muestra % Ubicado como valor principal y piezas como detalle.
- Tabla diaria: conserva Acondicionado en piezas y sustituye Ubicado en piezas por % Ubicado.
- PDF de Operación Diaria replica exactamente esa lectura.

No altera fórmulas ni datos del backend.
"""
...
def install(m):
    if getattr(m, "_V111_DAILY_PERCENT_FINAL", False):
        return

    from fastapi.responses import HTMLResponse

    css = r'''
<style id="v111-daily-percent-final-css">
...
'''
    ...
    js = r'''
<script id="v111-daily-percent-final-js">
(function(){
  ...
  function updateDailyCards(){
    ...
    document.querySelectorAll('#operativoDynamicContent .report-kpi').forEach(card=>{
      ...
      if(key==='acondicionado' || key==='% acondicionado'){
        ...
        if(!card.dataset.v111Pct){
          const originalPct=parsePercent(sub?.textContent||'');
...
'''
    ...
    async def _v111_html(request, call_next):
        ...
        try:
            ...
            if "v111-daily-percent-final-js" not in html:
    ...
    # PDF Diario alineado con la pantalla.
    ...
    def build_pdf(d:dict,report:str,scope:str="Compañía")->bytes:
        ...
        for i,r in enumerate(rows):
            ...
            if p_ub is None:p_ub=(num(r.get("ubicado"))/num(total)*100) if num(total)>0 else 0
            vals=[f"#{i+1}",r.get("store",""),f(r.get("dev_pzs")),f(r.get("muertos")),f(r.get("probador")),f(r.get("cajas")),f(r.get("pendiente_anterior")),f(total),f(r.get("recorridos")),f(r.get("acondicionado")),pct(p_ub),f(r.get("pendiente_acondicionar")),f(r.get("pendiente_ubicar"))]
    ...
    try:
        ...
        print("[V111-SELFTEST] Tarjetas 42.6%/94.6% y tabla % Ubicado OK.",flush=True)
    except Exception as exc:
        print(f"[V111-SELFTEST] ERROR: {type(exc).__name__}: {exc}",flush=True)

    m._V111_DAILY_PERCENT_FINAL=True
    print("[V111] Operación Diaria: % Acondicionado y % Ubicado en tarjetas; sólo % Ubicado en tabla.",flush=True)
